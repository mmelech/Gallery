 ...
 */
final class Version20220908091525 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        , CONSTRAINT FK_5F9E962A4B89032C FOREIGN KEY (post_id) REFERENCES posts (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_5F9E962AF675F31B FOREIGN KEY (author_id) REFERENCES users (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__posts AS SELECT id, category_id, date, title, content FROM posts');
        ...
        , title VARCHAR(120) NOT NULL, content CLOB NOT NULL, CONSTRAINT FK_885DBAFA12469DE2 FOREIGN KEY (category_id) REFERENCES categories (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO posts (id, category_id, date, title, content) SELECT id, category_id, date, title, content FROM __temp__posts');
        ...
        $this->addSql('DROP INDEX IDX_5ACE3AF0D5ECAD9F');
        $this->addSql('CREATE TEMPORARY TABLE __temp__post_tag AS SELECT posts_tags, tag_id FROM post_tag');
        $this->addSql('DROP TABLE post_tag');
        $this->addSql('CREATE TABLE post_tag (posts_tags INTEGER NOT NULL, tag_id INTEGER NOT NULL, PRIMARY KEY(posts_tags, tag_id), CONSTRAINT FK_5ACE3AF0D5ECAD9F FOREIGN KEY (posts_tags) REFERENCES posts (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_5ACE3AF0BAD26311 FOREIGN KEY (tag_id) REFERENCES tags (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO post_tag (posts_tags, tag_id) SELECT posts_tags, tag_id FROM __temp__post_tag');
